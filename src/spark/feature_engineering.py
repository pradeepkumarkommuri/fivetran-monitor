...
def engineer_features(df: DataFrame) -> DataFrame:
    ...
    w_ct = Window.partitionBy("connector_id", "table_name").orderBy("sync_time")
    w_7d = w_ct.rowsBetween(-7, -1)
    w_30d = w_ct.rowsBetween(-30, -1)

    return (
        df.withColumn("rows_lag1", F.lag("rows_updated", 1).over(w_ct))
        .withColumn("rows_lag2", F.lag("rows_updated", 2).over(w_ct))
        .withColumn("rows_lag3", F.lag("rows_updated", 3).over(w_ct))
        .withColumn(
            "rows_delta",
            F.col("rows_updated")
            - F.coalesce(F.col("rows_lag1"), F.col("rows_updated")),
        )
        .withColumn(
            "rows_pct_change",
            F.when(
        ...
        .withColumn(
            "z_score",
            F.when(
                F.col("rolling_std_30d") > 0,
                (F.col("rows_updated") - F.col("rolling_mean_30d"))
                / F.col("rolling_std_30d"),
            )
            .otherwise(F.lit(0.0))
            .cast(DoubleType()),
        )
...
def run_feature_job(partition: str) -> str:
    """Main entry point. partition format: YYYY/MM/DD/HH"""
...
if __name__ == "__main__":
    ...
    parser.add_argument("--partition", required=True, help="e.g. 2024/01/15/14")
    args = parser.parse_args()
    print(f"Output: {run_feature_job(args.partition)}")
