...
def run_training() -> dict:
    ...
    summary = {"isolation_forest": {"trained": 0, "skipped": 0},
    ...
    # ── Isolation Forest: one model per connector+table ───────────────────────
    for (cid, table), group in df.groupby(["connector_id", "table_name"]):
        with metrics.model_train_duration.labels(model_type="isolation_forest").time():
            model = anomaly_detector.train(cid, table, group)
        if model:
            summary["isolation_forest"]["trained"] += 1
            metrics.model_last_trained.labels(connector_id=cid).set(
                datetime.now(timezone.utc).timestamp()
            )
        else:
            summary["isolation_forest"]["skipped"] += 1

    # ── Prophet: one model per connector (aggregate rows across tables) ───────
    connector_ts = (
    ...
    for cid, group in connector_ts.groupby("connector_id"):
        with metrics.model_train_duration.labels(model_type="prophet").time():
            model = forecaster.train(cid, group)
        if model:
    ...
    log.info(
        "Training complete — IF: %d trained / %d skipped | "
        "Prophet: %d trained / %d skipped",
