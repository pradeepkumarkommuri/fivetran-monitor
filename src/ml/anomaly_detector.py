...
def train(connector_id: str, table: str, df: pd.DataFrame) -> Pipeline | None:
    """Train an Isolation Forest. Returns None if insufficient data."""
    cfg = get_settings().ml
    ...
    pipeline.fit(X)
    log.info(
        "Trained IsolationForest for %s/%s on %d samples",
        connector_id,
        table,
        len(X),
    )
