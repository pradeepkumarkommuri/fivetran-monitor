...
class Alert:
    connector_id:   str
    connector_name: str
    alert_type:     str       # row_anomaly | connector_health | ml_anomaly | ml_forecast
    issue:          str       # broken | delayed | paused | rescheduled | drop | spike | zero
...
def check_connector_health(connector: dict) -> list[Alert]:
    from src.ingestion.fivetran_client import FivetranClient  # noqa: avoid circular
    ...
    if sync_state in broken_states or update_state in broken_states:
        last_err = _parse(failed_at)
        down = f" (failing ~{int((now-last_err).total_seconds()//3600)}h)" if last_err else ""
        alerts.append(Alert(cid, cname, "connector_health", "broken", "critical",
            f"Connector in '{sync_state}'{down}. Last success: {succeeded_at or 'never'}"))
    elif paused or sync_state == "paused":
        alerts.append(Alert(cid, cname, "connector_health", "paused", "warning",
            f"Connector paused. Last sync: {succeeded_at or 'never'}"))
    elif sync_state == "rescheduled":
        alerts.append(Alert(cid, cname, "connector_health", "rescheduled", "warning",
    ...
    if sync_freq and succeeded_at and not paused and sync_state not in broken_states:
        last_ok = _parse(succeeded_at)
        if last_ok and (now - last_ok) > timedelta(minutes=sync_freq * cfg.delay_multiplier):
            elapsed = int((now - last_ok).total_seconds() // 60)
            alerts.append(Alert(cid, cname, "connector_health", "delayed", "warning",
...
def check_row_anomalies(connector: dict, history: list[dict]) -> list[Alert]:
    ...
    cid   = connector["id"]
    cname = connector.get("schema", cid)
    ok    = [h for h in history if h.get("status","").upper()=="SUCCESSFUL" and h.get("data")]
    if len(ok) < 2: return alerts
    cur, prev = ok[0].get("data",{}), ok[1].get("data",{})
    for table in set(cur) | set(prev):
        ...
        if cfg.zero_rows and cr == 0 and pr > 0:
            alerts.append(Alert(cid, cname, "row_anomaly", "zero", "critical",
        ...
        if pct <= -cfg.drop_pct:
            ...
            alerts.append(Alert(cid, cname, "row_anomaly", "drop", sev,
                f"Row count dropped {abs(pct):.1f}%",
                {"table": table, "previous": pr, "current": cr, "pct_change": pct}))
        elif pct >= cfg.spike_pct:
            alerts.append(Alert(cid, cname, "row_anomaly", "spike", "warning",
...
def load_ml_alerts() -> list[dict]:
    ...
    return [dict(zip(
        ["connector_id","table_name","scored_at","rows_updated",
         "anomaly_score","forecast_value","forecast_lower","forecast_upper","alert_reasons"],
        r
    )) for r in rows]
    ...
    with get_db_session() as session:
        count = session.execute(text("""
            ...
            WHERE connector_id = :cid
              ...
              AND fired_at     >= :window
        """), {"cid": alert.connector_id, "issue": alert.issue,
               "atype": alert.alert_type, "window": window}).scalar()
    return count > 0
...
def _log_alert(alert: Alert) -> None:
    with get_db_session() as session:
        session.execute(text("""
            INSERT INTO alert_log (connector_id, connector_name, alert_type,
                                   issue, severity, reason, details, fired_at)
            VALUES (:cid, :cname, :atype, :issue, :sev, :reason, :details, :fired_at)
        """), {
            "cid":      alert.connector_id,  "cname":   alert.connector_name,
            "atype":    alert.alert_type,    "issue":   alert.issue,
            "sev":      alert.severity,      "reason":  alert.reason,
            "details":  str(alert.details),  "fired_at": alert.fired_at,
        })
...
def dispatch_all(all_alerts: dict) -> int:
    ...
    # Reconstruct Alert objects from XCom dicts
    ...
    # Fire individual alerts with deduplication
    for alert in all_alert_objs:
        ...
        try:
            ...
            metrics.alerts_fired.labels(
                severity=alert.severity, type=alert.alert_type, channel="all"
            ).inc()
            sent += 1
        except Exception as exc:
            log.error("Failed to dispatch alert %s: %s", alert.issue, exc)

    # ML alerts (already in DB, just notify)
    for ml in all_alerts.get("ml", []):
        a = Alert(
            connector_id=ml["connector_id"], connector_name=ml.get("connector_id",""),
            alert_type="ml_anomaly", issue="ml_anomaly", severity="warning",
    ...
    log.info("Dispatched %d alerts (%d total before dedup)", sent, len(all_alert_objs))
    return sent
